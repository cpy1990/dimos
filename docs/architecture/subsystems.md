...
- [14. stream/](#14-stream)
- [附录 A：实验模块（experimental/）](#附录-a实验模块experimental)
...
**硬依赖子系统清单**（不装 `--extra perception` 则以下子系统 ImportError）：
- `agents.skills.navigation`
- `manipulation/`（`PickAndPlaceModule`、`scan_objects` 路径）
- `models/vl/*`（VLM / 检测共用 `perception.detection` 数据类型）
...
**VLM → bbox 桥接** — `navigation/visual/query.py` 暴露 `get_object_bbox_from_image(vl_model, image, object_description)`（`visual/query.py:21`），被 `skills/visual_navigation_skills.py` 调用，把 VLM 自然语言输出转为可用的 bbox，送入上述视觉伺服流水线。
...
flowchart TD
  ...
  WM[WorldMonitor<br/>planning/monitor/world_monitor.py]
  WOM[WorldObstacleMonitor<br/>planning/monitor/world_obstacle_monitor.py]
  RSM[RobotStateMonitor<br/>planning/monitor/robot_state_monitor.py]
  ...
  PLAN -->|uses| KIN
  PLAN -->|uses| WORLD
  WM -->|world state| WORLD
  WOM -->|obstacles| WORLD
  RSM -->|joint state| WORLD
  PAP -->|inherits| MM
...
**中层——规划层**（`planning/`）：后端无关设计的核心。`PlannerSpec`（`planning/spec/`）是 Protocol，`RRTConnectPlanner`（`planning/planners/rrt_planner.py`）是实现之一，只依赖 `WorldSpec` 接口做碰撞检测和 FK，不直接依赖 Drake。`KinematicsSpec` 同理：`JacobianIK`（迭代/微分两种实现）和 `drake_optimization_ik.py` 都实现此接口。`JointTrajectoryGenerator`（`planning/trajectory_generator/`）将关节路径插值为时间参数化轨迹。`manipulation/planning/monitor` 子包下 3 个监视器并存：`WorldMonitor`（`world_monitor.py:51`，整体世界状态快照）、`WorldObstacleMonitor`（`world_obstacle_monitor.py:48`，动态障碍物注入 World）、`RobotStateMonitor`（`robot_state_monitor.py:48`，机器人关节/末端状态订阅）。

**伺服控制（Cartesian 空间）** — `manipulation/control/servo_control/cartesian_motion_controller.py`：`CartesianMotionController(Module)`（`cartesian_motion_controller.py:87`）跑伺服循环，`CartesianMotionControllerConfig(ModuleConfig)`（`cartesian_motion_controller.py:53`）是配置；与 `trajectory_controller/`（关节空间路径）并列。
...
**视觉语言模型**（`vl/`）：`QwenVlModel` 和 `QwenModel`（`models/qwen/`）是本地 GPU 推理实现；`OpenAIVLM` 调用远程 API（GPT-4o、GPT-4V）；`Florence2`、`Moondream` 提供轻量检测/描述选项；`vl/moondream_hosted.py` 是 Moondream 的托管 API 变体（走远程 HTTP 端点，免本地权重）。
...
**`websocket_vis_spec.py`** — 暴露 `WebsocketVisSpec(Spec, Protocol)`（`websocket_vis_spec.py:21`），标准 SpecProtocol，让 IDE / 调试 UI 按类型连线消费可视化流，无需硬编码 topic 名。
...
**G1 wholebody 相关新增消息**（随 PR #1954 入库）：
- `sensor_msgs/JointCommand.py` — 单关节目标指令
- `sensor_msgs/MotorCommandArray.py` — 电机指令数组（G1 全身控制主要载体，见 [robot-platforms §1.2](/docs/architecture/robot-platforms.md#12-g1)）
- `sensor_msgs/RobotState.py` — 机器人整体状态快照
- `sensor_msgs/image_impls/` — 图像多后端实现（CPU / GPU / 共享内存）

**`trajectory_msgs/` 当前清单**：`JointTrajectory.py` / `TrajectoryPoint.py` / `TrajectoryStatus.py`——关节空间路径 + 执行状态回报。
...
**`manipulation.py`**：操控域类型库，共 **9 类**——`ConstraintType`（枚举）、`AbstractConstraint`（基类）、`TranslationConstraint` / `RotationConstraint` / `ForceConstraint`（3 种约束）、`ObjectData` / `ManipulationMetadata`（TypedDict）、`ManipulationTaskConstraint` + `ManipulationTask`（任务封装）。是 [`skills/manipulation/`](./agent-stack.md#skillsmanipulation--约束化机械臂动作新增) 的约束类型源头；OpenArm 机械臂随 PR 入库，见 [robot-platforms §4.3](/docs/architecture/robot-platforms.md#43-openarm-双臂)。

**`robot_capabilities.py`**：定义 `RobotCapability(Enum)`，列举平台能力标签（locomotion / manipulation / vision / audio…），`BlueprintConfig.required_capabilities` 与硬件匹配用。

**`constants.py` / `sample.py` / `ros_polyfill.py`**：上述已覆盖；`constants.py` 集中项目级常量（默认端口、帧率、物理单位换算）。
...
> 做什么见 [README § 6.14](/docs/architecture/README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。

**与 `dimos/core/stream.py` 区分**：`core/stream.py` 是 DimOS Stream 原语（`Stream[T]` 类、subscribe / pipe 骨架）；`dimos/stream/` 是**视频 / 音频源 + 算子组合**的高层工厂——把 RTSP、ROS、本地文件等异构输入统一包成 `Observable`。

### 内部架构

```
stream/
├── audio/                 ← 音频源（麦克风 / 文件 / TTS 输入）
├── data_provider.py       ← 通用数据 provider 抽象
├── video_provider.py      ← 基类 VideoProvider（本地文件 / 摄像头）
├── rtsp_video_provider.py ← RTSP 流 provider（ffmpeg / GStreamer 后端）
├── ros_video_provider.py  ← ROS 2 image topic 订阅 provider
├── frame_processor.py     ← 单帧预处理（resize / encode / 质量过滤）
├── stream_merger.py       ← 多路 Stream 合并（时间对齐 / zip / round-robin）
└── video_operators.py     ← RxPY ops 组：抽帧、跳帧、批处理、节流
```

### 依赖

- **上游**：`core/stream.py`（Stream 原语）、`rxpy_backpressure/`（背压）、`msgs/sensor_msgs/Image.py`（帧类型）、外部 `opencv` / `ffmpeg` / `rclpy`（可选）。
- **下游**：`perception/`、`memory/video/`、`web/`（通过 `WebsocketVis`/`websocket_vis_spec.py` 推流）、`visualization/rerun/`。
- **共享 Spec**：无专属 Spec；消费者通常声明 `Stream[Image]`。
- **共享 Stream**：`color_image` / `depth_image` / `audio_frames` 的主要源头。

### 扩展点

| 想加什么 | 改哪里 | 模式 |
|---|---|---|
| 新视频源（GigE / HDMI capture） | 继承 `VideoProvider`，实现 `__next__()` | 子类 |
| 新算子（锐度增强、ROI 裁剪） | 在 `video_operators.py` 加 RxPY operator | 算子组合 |
| 多源时间对齐策略 | 扩展 `stream_merger.py` | 策略扩展 |

---

## 附录 A：实验模块（`dimos/experimental/`）

API **不稳定**、不进 blueprint registry、不编号——因不稳定。当前仅一个子包：

**`security_demo/`：**
- `depth_estimator.py` — 深度估计
- `security_module.py` — 安防模块主体
- `conftest.py` + `test_security_module.py` — pytest 固件与测试

（曾计划中的 `temporal_memory/`（PR #1511 spatio-temporal RAG）在 `origin/main` 现态**不存在**于 `experimental/`，如有需要参考 PR 讨论。）
