...
> 与 [README § 6](README.md#-6-能力子系统全景) 互补：README §6 讲"做什么"；本文档讲"内部如何协作 + 依赖 + 扩展点"。**不重复** README §6 的"做什么"叙述。
...
> 做什么见 [README § 6.1](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.2](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
flowchart TD
  ...
  SP[SpatialPerception<br/>spatial_perception.py]
...
**第三层——空间层**：`spatial_perception.py`（`SpatialPerception` Module）在 `ObjectScene` 基础上维护一个空间向量数据库（ChromaDB），提供 `find_object(query)` 等 RPC。实验性的 `experimental/temporal_memory/` 在此之上加入了时间维度的帧窗口分析（`FrameWindowAccumulator`）、实体图谱（`EntityGraphDB`）和 CLIP 语义过滤。
...
> 做什么见 [README § 6.3](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.4](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.5](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
| 新地理地图源（Bing Maps） | 仿照 `google_maps.py` 实现新模块，共享 `mapping/types.py` 类型 | 同接口实现 |
...
> 做什么见 [README § 6.6](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.7](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
flowchart LR
  ...
  VQ[VideoQuery<br/>vl/video_query.py]
...
3. **接口分层**：`EmbeddingModel` 约定 `embed(image) -> Embedding`（固定维度浮点向量）；`Captioner` 约定 `caption(image) -> str`；`VLMBase` 在 `Captioner` 之上增加 `query(image, prompt) -> str` 和检测接口。`VideoQuery`（`vl/video_query.py`）在 VLM 上封装视频帧采样和聚合逻辑，用于长时序场景理解。
...
> 做什么见 [README § 6.8](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.9](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。

### 内部架构

```
visualization/rerun/
├── bridge.py        ← RerunBridge Module（RPC 接口 + LCM 订阅 + 节流）
└── test_viewer_integration.py
```

`RerunBridge`（`bridge.py`）是本子系统的唯一 Module。它通过 LCM pubsub 层订阅 glob 模式的消息频道（如 `/**`），对每个到达的消息调用 `to_rerun()` 方法将其转换为 Rerun SDK 的 archetypes（`rr.Image`、`rr.Points3D` 等），再调用 `rr.log()` 写入 Rerun gRPC 服务（端口 9876）或 Web 服务（端口 9090）。
...
**消息路由**：`RerunBridge` 通过 `Glob`/`pattern_matches`（来自 `dimos/protocol/pubsub/patterns.py`）实现灵活的订阅过滤，无需修改源 Module 即可选择性可视化任意 Stream。

`to_rerun()` 方法按约定部署在各 `dimos/msgs/` 消息类型中（如 `Image.to_rerun()`、`PointCloud2.to_rerun()`），遵循 `HasToRerun` Protocol 进行鸭子类型检查。
...
| 新订阅模式（只看特定传感器） | 修改 `RerunBridge` glob 配置 | 配置驱动 |
...
> 做什么见 [README § 6.10](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.11](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
`protocol.py` 中 `DimosMsg` 是 `runtime_checkable` Protocol，允许 `isinstance(msg, DimosMsg)` 动态检查，用于 `RerunBridge` 的消息路由和 `web/` 的 Stream 类型过滤。

**`to_rerun()` 约定**：凡需要可视化的消息类型（`Image`、`PointCloud2`、`Path` 等）均实现 `to_rerun() -> list[rr.AnyComponentBatch]` 方法，`RerunBridge` 通过 `HasToRerun` Protocol 鸭子类型检测是否可以直接 log。
...
> 做什么见 [README § 6.12](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
> 做什么见 [README § 6.13](README.md#-6-能力子系统全景)。本节聚焦内部架构、依赖、扩展点。
...
- 运行时模型：[`docs/architecture/runtime-model.md`](runtime-model.md)
- 数据流：[`docs/architecture/data-flow.md`](data-flow.md)
- Agent 系统：[`docs/architecture/agent-stack.md`](agent-stack.md)
