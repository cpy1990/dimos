...
`WorkerManager`（`dimos/core/coordination/worker_manager.py`）与 `ModuleCoordinator`（`dimos/core/coordination/module_coordinator.py`）是 DimOS 进程管理层的两个责任分明的角色，不能混淆。
