...
`dimos/core/transport.py` 共定义 **9 个**以 `Transport` 结尾的具名类（另有一个条件编译的 `DDSTransport`，见后文说明）：一个内部基类 `PubSubTransport`、主用的六个具体传输类、针对视频帧优化的两个 JPEG 变体，以及一个尚未落地的占位类。
...
- **`DDSTransport`**：基于 CycloneDDS，在文件末尾以 `if DDS_AVAILABLE:` 条件编译定义，因此 `grep '^class'` 的计数（9）不包含它——实际上一共有 10 个 Transport 具名类。DDS 支持丰富的 QoS 策略，是 Unitree 真机产线的首选传输；未安装 `cyclonedds` 时该类不存在，代码依然可运行。

**JPEG 视频变体**：`JpegLcmTransport` 和 `JpegShmTransport` 分别继承自 `LCMTransport` 和 `SHMTransport`，内部替换了底层 pub/sub 实现为 `JpegLCM` 与 `JpegSharedMemory`，在传输前对图像帧做 JPEG 压缩编码，目的是降低跨进程传输视频流时的带宽占用与 CPU 拷贝开销。两者专为图像流场景设计，普通非图像消息应使用基础变体。
...
| `JpegLcmTransport` / `JpegShmTransport` | 同基类 | 同基类 | 部分 | 视频帧流传输 | JPEG 压缩降低带宽 |
